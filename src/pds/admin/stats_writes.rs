//! Admin writes statistics page handler.
//!
//! Displays statistics for writes only (names starting with "apply_writes").
...
pub async fn admin_stats_writes(
    ...
    // Get writes statistics sorted by last updated (newest first)
    let mut statistics = state.db.get_writes_statistics().unwrap_or_default();
    statistics.sort_by(|a, b| b.last_updated_date.cmp(&a.last_updated_date));

    let html = build_stats_writes_page(&hostname, &statistics);

    Html(html).into_response()
}

/// Build the writes-stats page HTML showing every writes statistic in one table.
fn build_stats_writes_page(hostname: &str, statistics: &[Statistic]) -> String {
    let total_rows = statistics.len();
    let stats_rows = build_writes_rows_html(statistics);
